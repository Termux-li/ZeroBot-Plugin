...
func init() {
	...
	// 上传一张图
	engine.OnPrefix("添加wife", chkAddWifePermission, zero.MustProvidePicture).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			name := ""
	...
	engine.OnPrefix("删除wife", zero.AdminPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			name := ""
			for _, elem := range ctx.Event.Message {
				if elem.Type == "text" {
